...
- Test observable behavior: native-to-JS mapping, subscription lifetime, observer
  snapshots and change delivery. Do not test implementation source text.
...
- Hinge status and angle are separate from layout geometry. Keep angles in radians
  and unavailable angles null. Never guess feature-to-sensor mappings on devices
  with multiple hinges or synthesize native posture thresholds.
- iOS UIHingeInteraction uses an SDK compile guard and runtime availability check.
  Older SDKs compile out the feature; older runtimes return an empty array.
